...
var ThemeZoomControl = L.Control.Zoom.extend({
	options: {
		className: 'egy-leaflet-control-zoom'
	},
...
var ThemeScreenControl = L.Control.Screen.extend({
	options: {
		className: 'egy-leaflet-control-screen'
	},
	initialize: function (options){
		L.Util.setOptions(this, options);
	},
	...
	_update: function (){
		var options = this.options;
		var map = this._map;
		var className = 'egy-leaflet-disabled';
...
var ThemeBookmarkControl = L.Control.Bookmark.extend({
	options:{
		className: 'egy-leaflet-control-bookmark'
	},
	initialize: function(options){
		L.setOptions(this, options);
	},
	_onClick : function(){
		var map = this._map, zoom = this.options.zoom, center = this.options.center, className = 'egy-leaflet-disabled';
	...
	_update: function (){
		...
		var map = this._map, className = 'egy-leaflet-disabled';
...
var MessageBoxControl = L.Control.extend({
	options: {
		...
		textButton: 'M',
		msgClassName: '',
		controlClassName: 'leaflet-control-messagebox'
	},
	...
	_initLayout: function(){
		var	className = this.options.controlClassName ? this.options.controlClassName : 'leaflet-control-messagebox',
			container = this._container = L.DomUtil.create('div', className),
...
/* Theme Message Box */
var ThemeMessageBox = L.Control.extend({
	options: {
		position: 'topright',
		collapsed: true,
		className: '',
	...
	_initLayout: function(){
		var	className = 'egy-leaflet-control-messagebox',
			container = this._container = L.DomUtil.create('div', className),
			collapsed = this.options.collapsed,
			android = false;
		...
		if(collapsed){
			link = this._link = L.DomUtil.create('a', className + '-link', container);
			link.href = '#';
			link.title = this.options.title || 'Message Box';
		}else{
			msg = this._msg = L.DomUtil.create('div', className + '-msg', container);
			(!this.options.className) || L.DomUtil.addClass(msg, this.options.className); 
		}
		if (L.Browser.touch) {
			(!collapsed) || L.DomEvent.on(container, 'click', L.DomEvent.stop);
			(!collapsed) ||	L.DomEvent.on(container, 'click', this._toggle, this);
...
L.Map.addInitHook(function () {
	if (this.options.messageBox) {
		this.messageBox = new themeMessageBox();
...
var ThemeControl = L.Control.extend({
	...
	_switch: function (){
		...
		var className = this.options.leaflet ? 'std-leaflet-control' : 'egy-leaflet-control';
		...
		if(leaflet){
			L.DomUtil.addClass(this._container, 'egy-leaflet-control');
			L.DomUtil.addClass(this._leafletLink, 'egy-leaflet-control-link');
		}
		else{
		...
		this.options.leaflet = !this.options.leaflet;
	},
